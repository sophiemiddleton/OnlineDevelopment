// Example for streaming random no field cosmics
...
void hideNodesByName(TGeoNode* n, const std::string& str, bool onOff, int _diagLevel, REX::REveTrans& trans,  REX::REveElement* holder, int maxlevel, int level, bool shift) {
 ...
 public:
    ...
    virtual void start(){
       timer->SetCommand("eventMng->stream()");
       timer->Start();
    }
...
void event_demo_timer()
{
   ...
   auto timer = new TTimer(5000);
   timer->SetCommand("DoNextEvent();");
   timer->Start();
